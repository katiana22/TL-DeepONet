...
class DataSet:
    ...
    def load_data(self):
        ...
        # Source data
        file = io.loadmat('./Data/Dataset1/Dataset_2crack')
        s = 100
        r = 1552
        ...
        F_test = (F_test - f_train_mean)/(f_train_std) 
        ...
        # print(np.max(np.max(F_train)))
        # print(np.min(np.min(F_train)))
        # sys.exit()
    ...
    def load_data_target(self):
        ...
        F_train_t = (F_train_t - f_train_mean)/(f_train_std) 
        F_test_t = np.reshape(f_test, (-1, s, s, 1))
        F_test_t = (F_test_t - f_train_mean)/(f_train_std) 
