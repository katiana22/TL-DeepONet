...
def main():
    ...
    data = DataSet(bs)
    x_train, f_train, ux_train, uy_train, Xmin, Xmax = data.minibatch()
    ...
    conv_1, W1, b1 = conv_model.conv_layer(conv_linear, filter_size_1, num_filters_1, stride, actn=tf.nn.relu)  
    ...
    loss = tf.reduce_mean(tf.square(ux_ph - ux_pred)) + tf.reduce_mean(tf.square(uy_ph - uy_pred))
    ...
    while n <= nmax:
        ...
        if n <10000:
            ...
            lr = 0.00005
        x_train, f_train, ux_train, uy_train, _, _ = data.minibatch()
        train_dict = {f_ph: f_train, ux_ph: ux_train, uy_ph:uy_train, learning_rate: lr}
        loss_, _ = sess.run([loss, train], feed_dict=train_dict)
    
        if n%1 == 0:
            test_id, x_test, f_test, ux_test, uy_test= data.testbatch(bs)
            ux_test_, uy_test_ = sess.run([ux_pred, uy_pred], feed_dict={f_ph: f_test})
            # ux_test, uy_test = data.decoder(ux_test, uy_test)
            # ux_test_, uy_test_ = data.decoder(uy_test_, uy_test_)
    ...
    # Save variables  
    W_l_,b_l_,W1_,b1_,W2_,b2_,W3_,b3_,W4_,b4_,Wf1_,bf1_,Wf2_,bf2_,Wf3_,bf3_ = \
        sess.run([W_l,b_l,W1,b1,W2,b2,W3,b3,W4,b4,Wf1,bf1,Wf2,bf2,Wf3,bf3])
    
    savedict_cnn = {'W_l':W_l_,'b_l':b_l_,'W1':W1_,'b1':b1_,'W2':W2_,'b2':b2_,'W3':W3_,'b3':b3_,'W4':W4_,'b4':b4_,\
    ...
    data_save = SaveData()
    num_test = 100
    data_save.save(sess, x_pos, fnn_model, W, b, Xmin, Xmax, u_B, f_ph, ux_ph, uy_ph, data, num_test, save_results_to, domain='source')
