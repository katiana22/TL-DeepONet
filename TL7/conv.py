...
class CNN:
    ...
    ######## Target model #########
    def conv_layer_target(self, x, w_, b_, stride, actn=tf.nn.relu):
        # Freeze conv layers
        weight = tf.constant(w_, dtype=tf.float32)
        bias = tf.constant(b_, dtype=tf.float32)
        layer = tf.nn.conv2d(x, weight, strides=[1, stride, stride, 1], padding='SAME')
        layer += bias
        return actn(layer)
    
    def fnn_layer_target(self, x, w_, b_, actn=tf.tanh, use_actn=False):
        # Trainable layers (initialization based on source model)
        w = tf.Variable(w_, dtype=tf.float32)
        b = tf.Variable(b_, dtype=tf.float32)
        A = tf.add(tf.matmul(x, w), b)
        if use_actn:
            A = actn(A)
        return A, w, b
