...
def main():
    ...
    # c2 = tf.Variable(tf.reshape(100.0,(1, 1)))
   ...
   # Load pre-trained variables (from source network)
    ...
    #regularizers = fnn_model.l2_regularizer(W) 
    ...
    ##############################
    # Loss function
    l2_loss = tf.reduce_mean(tf.square(u_ph - u_pred)) + \
           ...
           tf.reduce_mean(tf.square(lambda_u0_t)*tf.square(u_ph[:,0:num,:] - u_pred[:,0:num,:])) 
    ceod_loss = loss2.CEOD([fnn_layer_1], u_ph, [fnn_layer_1_ph], u_pred_ph_s)   
    
    hybrid_loss = l2_loss + 10*ceod_loss
    ...
    # optimizer3 = tf.train.AdamOptimizer(learning_rate=5.0e-1, beta1 = 0.99, beta2 = 0.99)
    ...
    grads_lambda_u0 = optimizer2.compute_gradients(hybrid_loss, [lambda_u0])
    grads_lambda_u6 = optimizer2.compute_gradients(hybrid_loss, [lambda_u6])
    grads_lambda_u7 = optimizer2.compute_gradients(hybrid_loss, [lambda_u7])
    # grads_c1 = optimizer2.compute_gradients(hybrid_loss, [c1])
    # grads_c2 = optimizer3.compute_gradients(hybrid_loss, [c2])
    ...
    # op_c2= optimizer3.apply_gradients(grads_c2_minus)
    ...
    while n <= nmax:
        ...
        loss_, c2_, lambda_u0_, lambda_u6_, lambda_u7_ = sess.run([hybrid_loss, c2, lambda_u0, lambda_u6, lambda_u7], feed_dict=train_dict)
